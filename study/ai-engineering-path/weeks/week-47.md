---
type: ai-eng-week
week: 47
phase: 8
dates: "Jul 5-11, 2027"
course: "(build week)"
project: "P8 SHIP + capstone eval dashboard"
hours_target: 3.5
hours_logged: 0
status: not-started
streak_before: 0
created: 2026-05-29
tags:
  - study
  - ai-engineering
  - weekly
  - phase-8
---

# Week 47 — Phase 8: Production

> Dates: **Jul 5-11, 2027**
> 🎓 Capstone: Polish + deploy the capstone. Spec: [[../projects/p9-capstone]]
> Course: **(build week)**
> Project: **P8 SHIP + capstone eval dashboard**
> Time budget: **3-4h**
> Project spec: [[../projects/p8-ci-eval-pipeline]]

---

## 🎯 This week's focus

> Threshold logic, PR comment, intentional regression demo. ✅ P8 done. Capstone eval dashboard wired.

---

## ✅ This week's goals

> Set on Monday. 3 max.

- [ ] **Course:** complete this week's lessons
- [ ] **Project:** P8 SHIP + capstone eval dashboard
- [ ] **Notes:** 1-2 concept notes

---

## 📅 Realistic plan (3-4h, kid + work)

| Day | Time | Activity |
|-----|------|----------|
| Mon eve | 30m | Course lesson (after bedtime) |
| Wed eve | 30m | Reading / notes |
| Sat AM | 1.5-2h | Project work (nap window) |
| Sun eve | 30m | Course wrap + retro |

---

## 📝 Session log

### Session 1 — _date_
- Duration:
- Did:
- Learned:

### Session 2 — _date_
- Duration:
- Did:
- Learned:

### Session 3 — _date_
- Duration:
- Did:
- Learned:

---

## 🧠 Concept notes captured

| Concept | Confidence (L/M/H) | Note |
|---------|--------------------|------|
| | | [[]] |

---

## 🛠️ Code / project progress

- Repo: (see [[../projects/p8-ci-eval-pipeline]] for repo name)
- Commits this week:
- Demo / screenshot:

---

## 💸 Cost & usage

- Tokens (in/out):
- Cost: $
- API calls:

---

## 📓 Weekly journal

> Write on Sunday. ≥1 paragraph. Future-you + interviewer will read this.

<write here>

---

## ✅ Sunday retro

- [ ] Hours logged: ___ / 3.5
- [ ] Goals hit: ___ / 3
- [ ] Streak intact? (Y/N)
- [ ] Updated [[../02-progress-tracker]]
- [ ] Next week's calendar blocked

### What worked
-

### What didn't
-

### Adjustment for next week
-

### Confidence in path (1-10): _

---

*Prev: [[week-46]] · Back to [[../02-progress-tracker]] · Next: [[week-48]]*
