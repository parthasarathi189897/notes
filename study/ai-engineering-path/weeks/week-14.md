---
type: ai-eng-week
week: 14
phase: 2
dates: "Nov 16-22, 2026"
original_target: "Nov 2-8, 2026"
course: "C5 Building Apps w/ VDBs (2/2)"
project: "P2 main build"
hours_target: 3.5
hours_logged: 0
status: not-started
streak_before: 0
created: 2026-05-29
tags:
  - study
  - ai-engineering
  - weekly
  - phase-2
---

# Week 14 — Phase 2: Embeddings

> Dates: **Nov 16-22, 2026**
> Course: **C5 Building Apps w/ VDBs (2/2)**
> 📚 Course: [Building Applications with Vector Databases](https://learn.deeplearning.ai/courses/building-applications-vector-databases/lesson/tl7on/introduction) — DeepLearning.AI + Pinecone
> Project: **P2 main build**
> Time budget: **3-4h**
> Project spec: [[../projects/p2-semantic-search]]

---

## 🎯 This week's focus

> Recommenders, anomaly detection; P2 retrieval working on 100+ docs

---

## ✅ This week's goals

> Set on Monday. 3 max.

- [ ] **Course:** complete this week's lessons
- [ ] **Project:** P2 main build
- [ ] **Notes:** 1-2 concept notes

---

## ✅ Done when

> The single checkable condition for this slot. Approved 2026-09-07.
> A written note is not a shipped artifact.

- [ ] A top-5 query returns file + snippet + score + section metadata; index time and query latency logged.

---

## 📅 Realistic plan (3-4h, kid + work)

| Day | Time | Activity |
|-----|------|----------|
| Mon eve | 30m | Course lesson (after bedtime) |
| Wed eve | 30m | Reading / notes |
| Sat AM | 1.5-2h | Project work (nap window) |
| Sun eve | 30m | Course wrap + retro |

---

## 📝 Session log

### Session 1 — _date_
- Duration:
- Did:
- Learned:

### Session 2 — _date_
- Duration:
- Did:
- Learned:

### Session 3 — _date_
- Duration:
- Did:
- Learned:

---

## 🧠 Concept notes captured

| Concept | Confidence (L/M/H) | Note |
|---------|--------------------|------|
| | | [[]] |

---

## 🛠️ Code / project progress

- Repo: (see [[../projects/p2-semantic-search]] for repo name)
- Commits this week:
- Demo / screenshot:

---

## 💸 Cost & usage

- Tokens (in/out):
- Cost: $
- API calls:

---

## 📓 Weekly journal

> Write on Sunday. ≥1 paragraph. Future-you + interviewer will read this.

<write here>

---

## ✅ Sunday retro

- [ ] Hours logged: ___ / 3.5
- [ ] Goals hit: ___ / 3
- [ ] Streak intact? (Y/N)
- [ ] Updated [[../02-progress-tracker]]
- [ ] Next week's calendar blocked

### What worked
-

### What didn't
-

### Adjustment for next week
-

### Confidence in path (1-10): _

---

*Prev: [[week-13]] · Back to [[../02-progress-tracker]] · Next: [[week-15]]*
